...
from MRGARN import EnhancedDREMModel, MRGARN
...
class DocREModel(nn.Module):
    ...
    def secondary_reasoning(self, initial_relations, entity_features, adjacency):
        if adjacency is None:
            adjacency = torch.zeros_like(initial_relations).to(entity_features.device)
        else:
            adjacency = adjacency.to_dense()

        if entity_features is None:
            entity_features = torch.zeros_like(initial_relations).to(initial_relations.device)
        num_entities = entity_features.size(1) if len(entity_features.shape) > 1 else 1
        if len(adjacency.shape) > 3:
            entity_adjacency = adjacency[:, -1, :num_entities, :num_entities]
        else:
            entity_adjacency = adjacency[:, :num_entities, :num_entities]

        if len(entity_features.shape) == 2:
            entity_features = entity_features.unsqueeze(0)
        if entity_adjacency.shape[-1] != entity_features.shape[-1]:
            entity_adjacency = entity_adjacency.transpose(-1, -2)
        if entity_features.shape[0] != entity_adjacency.shape[0]:
            entity_adjacency = entity_adjacency.expand(entity_features.shape[0], -1, -1)
        secondary_relations = torch.einsum('bni,bnj->bnij', entity_features, entity_adjacency)
        if secondary_relations.shape != initial_relations.shape:
            try:
                secondary_relations = secondary_relations.view(-1)
                secondary_relations = secondary_relations[:initial_relations.numel()].view(initial_relations.shape)
            except RuntimeError:
                print(
                    f"Failed to reshape secondary_relations from {secondary_relations.shape} to {initial_relations.shape}")
                raise
    ...
    def secondary_reasoning_loss(self, initial_relations, entity_features, adjacency, labels=None):
        ...
        if labels is not None:
            ...
            raise ValueError("Labels are required to calculate the secondary reasoning loss.")
    ...
    def forward(self,
        ...
        for i in range(num_entities):
            ...
            if entity_i_features:
                entity_i_features = torch.stack(entity_i_features)
                mdeia_entity_features.append(entity_i_features)
        if mdeia_entity_features:
            mdeia_entity_features = torch.stack(mdeia_entity_features)
            if mdeia_entity_features.size(2) == 0:
                mdeia_entity_features = mdeia_entity_features.view(mdeia_entity_features.size(0), -1,
                                                                   mdeia_entity_features.size(-1))
            if mdeia_entity_features.size(1) == nodes.size(1):
